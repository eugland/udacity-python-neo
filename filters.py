...
def create_filters(
    ...
    `--date` option corresponds to the `date` argument, and represents
    a filter that selects close approaches that occured on exactly that
    given date. Similarly, the `--min-distance` option corresponds to the
    ...
    from Earth. Each option is `None` if not specified at the command line
    (in particular, this means that the `--not-hazardous` flag results in
    ...
    if start_date is not None:
        filters_map.append(Date(operator.ge, start_date))
    if end_date is not None:
        filters_map.append(Date(operator.le, end_date))
    if distance_min is not None:
        filters_map.append(Distance(operator.ge, distance_min))
    if distance_max is not None:
        filters_map.append(Distance(operator.le, distance_max))
    if velocity_min is not None:
        filters_map.append(Velocity(operator.ge, velocity_min))
    if velocity_max is not None:
        filters_map.append(Velocity(operator.le, velocity_max))
    if diameter_min is not None:
        filters_map.append(Diameter(operator.ge, diameter_min))
    if diameter_max is not None:
        filters_map.append(Diameter(operator.le, diameter_max))
    if hazardous is not None:
        filters_map.append(Hazardous(operator.is_, hazardous))
    return filters_map
